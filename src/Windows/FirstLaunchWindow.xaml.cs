...
partial class FirstLaunchWindow : TEKWindow
{
	...
	void Continue(object sender, RoutedEventArgs e)
	{
		if (_path!.StartsWith(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile)) && !Messages.ShowOptions("Warning", LocManager.GetString(LocCode.GamePathInUserFolder)))
			return;
		if (sender == _beginInstallationButton && Directory.EnumerateFileSystemEntries(_path).Any() && !Messages.ShowOptions("Warning", LocManager.GetString(LocCode.GameDirNotEmpty)))
			return;
		Settings.PreAquatica = _preAquatica;
		Game.Path = _path;
	...
	void GameInstallationTabLoadedHandler(object sender, RoutedEventArgs e)
	{
		...
		_reqSpace = (Run)template.FindName("ReqSpace", Root);
		_ver1 = (CheckBox)template.FindName("ver1", Root);
		_ver2 = (CheckBox)template.FindName("ver2", Root);
	}
	...
	void GIPathChangedHandler(string newPath)
	{
		...
		bool enoughSpace = freeSpace >= (_preAquatica ? 138512695296 : 178241142784);
	...
	void SetVer(object sender, RoutedEventArgs e)
	{
		_preAquatica = sender == _ver1;
		(_preAquatica ? _ver2 : _ver1).IsChecked = false;
		_reqSpace.Text = _preAquatica ? "129" : "166";
		if (string.IsNullOrEmpty(_path))
			return;
		long freeSpace = WinAPI.GetDiskFreeSpace(_path);
		bool enoughSpace = freeSpace >= (_preAquatica ? 138512695296 : 178241142784);
		_freeSpace.Foreground = new SolidColorBrush(enoughSpace ? Color.FromRgb(0x0A, 0xA6, 0x3E) : Color.FromRgb(0x9E, 0x23, 0x13));
		_freeSpace.Text = LocManager.BytesToString(freeSpace);
		_beginInstallationButton.IsEnabled = enoughSpace;
	...
	void PSPathChangedHandler(string newPath)
	{
		...
		if (filesExist)
		{
			_preAquatica = !Directory.Exists($@"{newPath}\ShooterGame\Content\Abyss");
			_path = newPath;
